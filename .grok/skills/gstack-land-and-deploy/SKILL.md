...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
- `/land-and-deploy #[object Object]23` — specific PR number
- `/land-and-deploy #[object Object]23 <url>` — specific PR + verification URL
...
**Always stop for:**
- **First-run dry-run validation (Step [object Object].5)** — shows deploy infrastructure and confirms setup
...
- **Never be robotic.** "I ran 4 checks and found [object Object] issue" not "CHECKS: 4, ISSUES: [object Object]."
...
[object Object]. Check GitHub CLI authentication:
```bash
gh auth status
```
...
## Step [object Object].5: First-run dry-run validation
...
if [ ! -f ~/.gstack/projects/$SLUG/land-deploy-confirmed ]; then
  ...
  CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f[object Object])
  # Also hash workflow files that affect deploy behavior
  WORKFLOW_HASH=$(find .github/workflows -maxdepth [object Object] \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f[object Object])
...
Then proceed to the FIRST_RUN flow below (steps [object Object].5a through [object Object].5e).
...
### [object Object].5a: Deploy infrastructure detection

Run the deploy configuration bootstrap to detect the platform and settings:

```bash
# Check for persisted deploy config in CLAUDE.md
DEPLOY_CONFIG=$(grep -A 2[object Object] "## Deploy Configuration" CLAUDE.md 2>/dev/null || echo "NO_CONFIG")
echo "$DEPLOY_CONFIG"

# If config exists, parse it
if [ "$DEPLOY_CONFIG" != "NO_CONFIG" ]; then
  PROD_URL=$(echo "$DEPLOY_CONFIG" | grep -i "production.*url" | head -[object Object] | sed 's/.*: *//')
  PLATFORM=$(echo "$DEPLOY_CONFIG" | grep -i "platform" | head -[object Object] | sed 's/.*: *//')
...
for f in $(find .github/workflows -maxdepth [object Object] \( -name '*.yml' -o -name '*.yaml' \) 2>/dev/null); do
...
### [object Object].5b: Command validation

Test each detected command to verify the detection is accurate. Build a validation table:

```bash
# Test gh auth (already passed in Step [object Object], but confirm)
gh auth status 2>&[object Object] | head -3

# Test platform CLI if detected
# Fly.io: fly status --app {app} 2>/dev/null
# Heroku: heroku releases --app {app} -n [object Object] 2>/dev/null
...
║  ├─ curl prod URL:      ✓ PASS (2[object Object][object Object] OK) / ⚠ UNREACHABLE   ║
...
║                                                            ║
║  WHAT WILL HAPPEN                                          ║
║  [object Object]. Run pre-merge readiness checks (reviews, tests, docs)  ║
║  2. Wait for CI if pending                                 ║
║  3. Merge PR via {merge method}                            ║
║  4. {Wait for deploy workflow / Wait 6[object Object]s / Skip}           ║
...
failed at Step [object Object]). If `curl` fails, note "I couldn't reach that URL — might be a network
...
### [object Object].5c: Staging detection

Check for staging environments in this order:

[object Object]. **CLAUDE.md persisted config:** Check for a staging URL in the Deploy Configuration section:
```bash
...
```bash
gh pr checks --json name,targetUrl 2>/dev/null | head -2[object Object]
```
...
### [object Object].5d: Readiness preview
...
```bash
~/.grok/skills/gstack/bin/gstack-review-read 2>/dev/null
```
...
### [object Object].5e: Dry-run confirmation
...
- Show the infrastructure validation table from [object Object].5b above.
...
- A) That's right — this is how my project deploys. Let's go. (Completeness: [object Object][object Object]/[object Object][object Object])
- B) Something's off — let me tell you what's wrong (Completeness: [object Object][object Object]/[object Object][object Object])
- C) I want to configure this more carefully first (runs /setup-deploy) (Completeness: [object Object][object Object]/[object Object][object Object])
...
Parse the output:
[object Object]. If any required checks are **FAILING**: **STOP.** "CI is failing on this PR. Here are the failing checks: {list}. Fix these before deploying — I won't merge code that hasn't passed CI."
...
If required checks are still pending, wait for them to complete. Use a timeout of [object Object]5 minutes:
...
If timeout ([object Object]5 min): **STOP.** "CI has been running for over [object Object]5 minutes — that's unusual. Check the GitHub Actions tab to see if something is stuck."
...
[object Object]. If `OFFLINE=true` or the util fails: print `⚠ VERSION drift check unavailable (util offline) — proceeding with PR version v<BRANCH_VERSION>`. Continue to Step 3.5. CI's version-gate job is the backstop.
...
3. If drift is detected (a PR landed ahead of us and `BRANCH_VERSION < NEXT_SLOT`): **STOP** and print exactly:
   ...
   Exit non-zero. Do NOT auto-bump from `/land-and-deploy` — rerunning `/ship` is the clean path (it already handles VERSION + package.json + CHANGELOG header + PR title atomically via Step [object Object]2 ALREADY_BUMPED detection).
...
[object Object]. Find the most recent entry within the last 7 days.
2. Extract its `commit` field.
3. Compare against current HEAD: `git rev-list --count STORED_COMMIT..HEAD`

**Staleness rules:**
- [object Object] commits since review → CURRENT
- [object Object]-3 commits since review → RECENT (yellow if those commits touch code, not just docs)
...
- A) Run a quick review (~2 min) — I'll scan the diff for common issues like SQL safety, race conditions, and security gaps (Completeness: 7/[object Object][object Object])
- B) Stop and run a full `/review` first — deeper analysis, more thorough (Completeness: [object Object][object Object]/[object Object][object Object])
- C) Skip the review — I've reviewed this code myself and I'm confident (Completeness: 3/[object Object][object Object])

**If A (quick checklist):** Tell the user: "Running the review checklist against your diff now..."

Read the review checklist:
```bash
cat ~/.grok/skills/gstack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
bun test 2>&[object Object] | tail -[object Object][object Object]
```
...
ls -t ~/.gstack-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -2[object Object]
```
...
```bash
git log --oneline $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main)..HEAD | head -2[object Object]
```

Compare the PR body against the actual commits. Check for:
[object Object]. **Missing features** — commits that add significant functionality not mentioned in the PR
...
║  ├─ VERSION:       [object Object].9.8.[object Object] / NOT BUMPED (warning)        ║
...
- A) Merge it — everything looks good (Completeness: [object Object][object Object]/[object Object][object Object])
- B) Hold off — I want to fix the warnings first (Completeness: [object Object][object Object]/[object Object][object Object])
- C) Merge anyway — I understand the warnings and want to proceed (Completeness: 3/[object Object][object Object])
...
**Universal invariant:** after ANY non-zero exit from `gh pr merge`, query authoritative PR state before retrying or stopping. Do NOT retry `gh pr merge`. Related: cli/cli#3442, cli/cli#[object Object]338[object Object].
...
Poll every 3[object Object] seconds, up to 3[object Object] minutes. Show a progress message every 2 minutes:
...
If timeout (3[object Object] min): **STOP.** "The merge queue has been processing for 3[object Object] minutes. Something might be stuck — check the GitHub Actions tab and the merge queue page."
...
```bash
eval $(~/.grok/skills/gstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
echo "FRONTEND=$SCOPE_FRONTEND BACKEND=$SCOPE_BACKEND DOCS=$SCOPE_DOCS CONFIG=$SCOPE_CONFIG"
```

**Decision tree (evaluate in order):**

[object Object]. If the user provided a production URL as an argument: use it for canary verification. Also check for deploy workflows.
...
If staging was detected in Step [object Object].5c (or from CLAUDE.md deploy config), and the changes
...
- A) Deploy to staging first, verify it works, then go to production (Completeness: [object Object][object Object]/[object Object][object Object])
- B) Skip staging — go straight to production (Completeness: 7/[object Object][object Object])
- C) Deploy to staging only — I'll check production later (Completeness: 8/[object Object][object Object])
...
```bash
gh run list --branch <base> --limit [object Object][object Object] --json databaseId,headSha,status,conclusion,name,workflowName
```
...
Poll every 3[object Object] seconds:
```bash
...
curl -sf {production-url} -o /dev/null -w "%{http_code}" 2>/dev/null
```
Render deploys typically take 2-5 minutes. Poll every 3[object Object] seconds.

**Heroku:** Check latest release:
```bash
heroku releases --app {app} -n [object Object] 2>/dev/null
```

### Strategy C: Auto-deploy platforms (Vercel, Netlify)

Vercel and Netlify deploy automatically on merge. No explicit deploy trigger needed. Wait 6[object Object] seconds for the deploy to propagate, then proceed directly to canary verification in Step 7.
...
If timeout (2[object Object] min): "The deploy has been running for 2[object Object] minutes, which is longer than most deploys take. The site might still be deploying, or something might be stuck." Ask whether to continue waiting or skip verification.
...
| SCOPE_CONFIG only | Smoke: `$B goto` + verify 2[object Object][object Object] status |
...
Check that the page loaded successfully (2[object Object][object Object], not an error page).
...
Check that page load time is under [object Object][object Object] seconds.
...
**Health assessment:**
- Page loads successfully with 2[object Object][object Object] status → PASS
- No critical console errors → PASS
- Page has real content (not blank or error screen) → PASS
- Loads in under [object Object][object Object] seconds → PASS
...
## Step [object Object][object Object]: Suggest follow-ups
...
- If a production URL was verified: "Want extended monitoring? Run `/canary <url>` to watch the site for the next [object Object][object Object] minutes."
...
- **Poll with backoff.** Don't hammer GitHub API. 3[object Object]-second intervals for CI/deploy, with reasonable timeouts.
