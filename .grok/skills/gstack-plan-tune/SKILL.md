...
name: plan-tune
description: |
  Self-tuning question sensitivity + developer psychographic for gstack (v[object Object]: observational).
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-tune","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
# /plan-tune — Question Tuning + Developer Profile (v[object Object] observational)
...
**v[object Object] scope (observational):** typed question registry, per-question explicit
...
Canonical reference: `docs/designs/PLAN_TUNING_V[object Object].md`.

---

## Step [object Object]: Detect what the user wants
...
[object Object]. **Consent gate.** If `question_tuning` is `false` AND
...
3. **Dream-cycle gate (Layer 8 / cathedral T[object Object][object Object]/T[object Object][object Object]).** If
...
[object Object][object Object]. **"Turn it off" / "disable"** → `~/.grok/skills/gstack/bin/gstack-config set question_tuning false`
[object Object][object Object]. **"Turn it on" / "enable"** → `~/.grok/skills/gstack/bin/gstack-config set question_tuning true && touch ~/.gstack/.question-tuning-prompted`
[object Object]2. **Clear ambiguity** — if you can't tell what the user wants, ask plainly:
...
**When this fires.** Step [object Object]'s consent gate: `question_tuning` is `false` AND
...
`docs/designs/PLAN_TUNING_V[object Object].md` §"Decisions log" for the privacy posture
...
[object Object]. Detect contributor state (for prompt framing only, not for auto-action):
   ```bash
   _QT=$(~/.grok/skills/gstack/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
   _CONTRIB=$(~/.grok/skills/gstack/bin/gstack-config get gstack_contributor 2>/dev/null || echo "false")
...
2. AskUserQuestion (use the contributor-specific framing only if `_CONTRIB=true`,
   ...
   **General framing:**
   ...
   > initial profile. v[object Object] is observational: gstack tracks your preferences
   ...
   >
   > RECOMMENDATION: Enable and set up your profile. Completeness: A=9/[object Object][object Object].
   >
   ...
   **Contributor framing (only if `_CONTRIB=true`):**
   ...
   > machine. v[object Object] is observational only.
   >
   > RECOMMENDATION: Enable and set up your profile. Completeness: A=9/[object Object][object Object].
   >
...
4. If A or B: enable:
   ```bash
   ~/.grok/skills/gstack/bin/gstack-config set question_tuning true
   ```
...
- Standalone via Step [object Object]'s setup gate: `question_tuning` is already `true`
...
[object Object]. Ask FIVE one-per-dimension declaration questions via individual
   AskUserQuestion calls (one at a time). Use plain English, no jargon:

   **Q[object Object] — scope_appetite:** "When you're planning a feature, do you lean toward
   shipping the smallest useful version fast, or building the complete, edge-
   case-covered version?"
   Options: A) Ship small, iterate (low scope_appetite ≈ [object Object].25) /
   B) Balanced / C) Boil the ocean — ship the complete version (high ≈ [object Object].85)

   **Q2 — risk_tolerance:** "Would you rather move fast and fix bugs later, or
   check things carefully before acting?"
   Options: A) Check carefully (low ≈ [object Object].25) / B) Balanced / C) Move fast (high ≈ [object Object].85)

   **Q3 — detail_preference:** "Do you want terse, 'just do it' answers or
   verbose explanations with tradeoffs and reasoning?"
   Options: A) Terse, just do it (low ≈ [object Object].25) / B) Balanced /
   C) Verbose with reasoning (high ≈ [object Object].85)

   **Q4 — autonomy:** "Do you want to be consulted on every significant
   decision, or delegate and let the agent pick for you?"
   Options: A) Consult me (low ≈ [object Object].25) / B) Balanced /
   C) Delegate, trust the agent (high ≈ [object Object].85)

   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   and 'get the design right', which side do you usually fall on?"
   Options: A) Ship now (low ≈ [object Object].25) / B) Balanced /
   C) Get the design right (high ≈ [object Object].85)
   ...
   ~/.grok/skills/gstack/bin/gstack-developer-profile --read >/dev/null
   ...
   bun -e "
     ...
     p.declared.scope_appetite = <Q[object Object]_VALUE>;
...
2. Touch the marker so the Setup gate doesn't re-fire:
   ...
   anytime with `/plan-tune setup` (Step [object Object] power-user shortcut).
...
```bash
~/.grok/skills/gstack/bin/gstack-developer-profile --profile
```
...
- For each dimension where `declared[dim]` is set, translate to a plain-English
  ...
  - [object Object].[object Object]-[object Object].3 → "low" (e.g., `scope_appetite` low = "small scope, ship fast")
  - [object Object].3-[object Object].7 → "balanced"
  - [object Object].7-[object Object].[object Object] → "high" (e.g., `scope_appetite` high = "boil the ocean")

  Format: "**scope_appetite:** [object Object].8 (boil the ocean — you prefer the complete
  version with edge cases covered)"

- If `inferred.diversity` passes the **display gate** (`sample_size >= 2[object Object] AND
  skills_covered >= 3 AND question_ids_covered >= 8 AND days_span >= 7`), show
  the inferred column next to declared:
  "**scope_appetite:** declared [object Object].8 (boil the ocean) ↔ observed [object Object].72 (close)"
  Use words for the gap: [object Object].[object Object]-[object Object].[object Object] "close", [object Object].[object Object]-[object Object].3 "drift", [object Object].3+ "mismatch".

  This display gate is intentionally lower than the E[object Object] **promotion gate**
  (9[object Object]+ days stable across 3+ skills, per `docs/designs/PLAN_TUNING_V[object Object].md`).
  ...
  bar. Do NOT use the display gate as a green light for v2 E[object Object] work.
  ...
  bun -e "
    ...
    for (const l of lines) {
      try {
        ...
        if (!byId[e.question_id]) byId[e.question_id] = { count:[object Object], skill:e.skill, summary:e.question_summary, followed:[object Object], overridden:[object Object] };
    ...
    for (const r of rows.slice([object Object], 2[object Object])) {
...
[object Object]. Identify the `question_id` from the user's words. If ambiguous, ask:
...
4. Write:
   ```bash
   ~/.grok/skills/gstack/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
boil-the-ocean than [object Object].5 suggests", "I've gotten more careful about architecture",
"bump detail_preference up".

**Always confirm before writing.** Free-form input + direct profile mutation
is a trust boundary (Codex #[object Object]5 in the design doc).

[object Object]. Parse the user's intent. Translate to `(dimension, new_value)`.
   - "more boil-the-ocean" → `scope_appetite` → pick a value [object Object].[object Object]5 higher than
     current, clamped to [[object Object], [object Object]]
   - "more careful" / "more principled" / "more rigorous" → `architecture_care`
     up
   - "more hands-off" / "delegate more" → `autonomy` up
   - Specific number ("set scope to [object Object].8") → use it directly
...
```bash
~/.grok/skills/gstack/bin/gstack-developer-profile --gap
```

Parse the JSON. For each dimension where both declared and inferred exist:

- `gap < [object Object].[object Object]` → "close — your actions match what you said"
- `gap [object Object].[object Object]-[object Object].3` → "drift — some mismatch, not dramatic"
- `gap > [object Object].3` → "mismatch — your behavior disagrees with your self-description.
  Consider updating your declared value, or reflect on whether your behavior
  is actually what you want."

Never auto-update declared based on the gap. In v[object Object] the gap is reporting only —
...
Cathedral T[object Object]3 surfaces: host-aware breakdown (claude hook vs codex import
vs agent-enriched), marked vs hash-only, auto-decided count, and dream
cycle cost-to-date.

```bash
~/.grok/skills/gstack/bin/gstack-question-preference --stats
eval "$(~/.grok/skills/gstack/bin/gstack-slug 2>/dev/null)"
eval "$(~/.grok/skills/gstack/bin/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
  bun -e "
    ...
    const bySource = {};
    let marked = [object Object];
    for (const e of events) {
      const src = e.source || 'agent';
      bySource[src] = (bySource[src] || [object Object]) + [object Object];
      if (e.question_id && !e.question_id.startsWith('hook-')) marked++;
    }
    console.log('TOTAL_LOGGED: ' + total);
    console.log('MARKED: ' + marked + ' (' + (total ? Math.round([object Object][object Object][object Object]*marked/total) : [object Object]) + '%)');
    for (const s of Object.keys(bySource).sort()) {
      console.log('SOURCE_' + s.toUpperCase().replace(/-/g,'_') + ': ' + bySource[s]);
    }
  "
else
  echo 'TOTAL_LOGGED: [object Object]'
fi
~/.grok/skills/gstack/bin/gstack-developer-profile --profile | bun -e "
  const p = JSON.parse(await Bun.stdin.text());
  const d = p.inferred?.diversity || {};
  console.log('SKILLS_COVERED: ' + (d.skills_covered ?? [object Object]));
  console.log('QUESTIONS_COVERED: ' + (d.question_ids_covered ?? [object Object]));
  console.log('DAYS_SPAN: ' + (d.days_span ?? [object Object]));
  console.log('CALIBRATED: ' + (p.inferred?.sample_size >= 2[object Object] && d.skills_covered >= 3 && d.question_ids_covered >= 8 && d.days_span >= 7));
"
echo '---DISTILL---'
~/.grok/skills/gstack/bin/gstack-distill-free-text --status
```
...
correction — without source columns, the cathedral's "before:[object Object] / after:>[object Object]"
claim is invisible).
...
Show the last [object Object][object Object] questions where the PreToolUse hook auto-decided (source=
...
[ ! -f "$_LOG" ] && echo 'NO_LOG' || bun -e "
  ...
  const recent = auto.slice(-[object Object][object Object]).reverse();
  if (!recent.length) { console.log('(no auto-decisions yet)'); process.exit([object Object]); }
...
the skill template — D[object Object]8 progressive markers). Surfacing them drives marker
  ...
  for (const l of lines) {
    try {
      ...
      if (e.question_id && e.question_id.startsWith('hook-')) {
        counts[e.question_id] = (counts[e.question_id] || [object Object]) + [object Object];
        summaries[e.question_id] = e.question_summary || '';
      }
    } catch {}
  }
  const rows = Object.entries(counts).sort((a,b) => b[[object Object]] - a[[object Object]]).slice([object Object], [object Object][object Object]);
  if (!rows.length) { console.log('(no unmarked questions — coverage is [object Object][object Object][object Object]%)'); process.exit([object Object]); }
...
**When this fires.** Step [object Object]'s dream-cycle gate: `distillation-proposals.json`
has at least one proposal with `applied_at` missing. Or the user explicitly
invokes via `/plan-tune distill` / `dream`.

**Flow:**

[object Object]. Show the proposals:
   ```bash
   ~/.grok/skills/gstack/bin/gstack-distill-apply --list
   ```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   ~/.grok/skills/gstack/bin/gstack-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   ~/.grok/skills/gstack/bin/gstack-distill-apply --proposal N
   ```
...
4. **On decline**: skip without marking. User can re-decide later (the
   ...
   `gstack-distill-apply --proposal N --dismiss` (not implemented in T[object Object][object Object];
...
`distill` / `dream cycle`. Auto-triggered version lives in Step [object Object] gate #3.

**Flow:**

[object Object]. Run distill:
   ```bash
   ~/.grok/skills/gstack/bin/gstack-distill-free-text
   ```
...
```bash
~/.grok/skills/gstack/bin/gstack-distill-free-text --background
```
...
- **Plain English everywhere.** Never require the user to know `profile set
  autonomy [object Object].4`. The skill interprets plain language; shortcuts exist for
  power users.
...
- **No behavior adaptation in v[object Object].** This skill INSPECTS and CONFIGURES. No
