...
description: |
  ...
  gauntlet without answering [object Object]5-3[object Object] intermediate questions. (gstack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"autoplan","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
[object Object]. **Choose completeness** — Ship the whole thing. Pick the approach that covers more edge cases.
2. **Boil lakes** — Fix everything in the blast radius (files modified by this plan + direct importers). Auto-approve expansions that are in blast radius AND < [object Object] day CC effort (< 5 files, no new infra).
3. **Pragmatic** — If two options fix the same thing, pick the cleaner one. 5 seconds choosing, not 5 minutes.
4. **DRY** — Duplicates existing functionality? Reject. Reuse what exists.
5. **Explicit over clever** — [object Object][object Object]-line obvious fix > 2[object Object][object Object]-line abstraction. Pick what a new contributor reads in 3[object Object] seconds.
6. **Bias toward action** — Merge > review cycles > stale deliberation. Flag concerns but don't block.

**Conflict resolution (context-dependent tiebreakers):**
- **CEO phase:** P[object Object] (completeness) + P2 (boil lakes) dominate.
- **Eng phase:** P5 (explicit) + P3 (pragmatic) dominate.
- **Design phase:** P5 (explicit) + P[object Object] (completeness) dominate.
...
[object Object]. **Close approaches** — top two are both viable with different tradeoffs.
...
[object Object]. Premises (Phase [object Object]) — require human judgment about what problem to solve.
...
State what you examined and why nothing was flagged ([object Object]-2 sentences minimum).
...
## Phase [object Object]: Intake + Restore Point

### Step [object Object]: Capture restore point
...
## Re-run Instructions
[object Object]. Copy "Original Plan State" below back to your plan file
2. Invoke /autoplan
...
- Read CLAUDE.md, TODOS.md, git log -3[object Object], git diff against the base branch --stat
- Discover design docs: `ls -t ~/.gstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -[object Object]`
...
- `~/.grok/skills/gstack/plan-ceo-review/SKILL.md`
- `~/.grok/skills/gstack/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/.grok/skills/gstack/plan-eng-review/SKILL.md`
- `~/.grok/skills/gstack/plan-devex-review/SKILL.md` (only if DX scope detected)
...
- Completion Status Protocol
- Telemetry (run last)
- Step [object Object]: Detect base branch
- Review Readiness Dashboard
- Plan File Review Report
...
## Phase [object Object].5: Codex auth + version preflight
...
source ~/.grok/skills/gstack/bin/gstack-codex-probe
...
If `_CODEX_AVAILABLE=false`, all Phase [object Object]-3.5 Codex voices below degrade to
...
## Phase [object Object]: CEO Review (Strategy & Scope)
...
- Alternatives: pick highest completeness (P[object Object]). If tied, pick simplest (P5).
  If top 2 are close → mark TASTE DECISION.
- Scope expansion: in blast radius + <[object Object]d CC → approve (P2). Outside → defer to TODOS.md (P3).
  Duplicates → reject (P4). Borderline (3-5 files) → mark TASTE DECISION.
- All [object Object][object Object] review sections: run fully, auto-decide each issue, log every decision.
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  _gstack_codex_timeout_wrapper 6[object Object][object Object] codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/gstack). These are AI assistant skill definitions meant for a different system. Stay focused on repository code only.

  You are a CEO/founder advisor reviewing a development plan.
  Challenge the strategic foundations: Are the premises valid or assumed? Is this the
  right problem to solve, or is there a reframing that would be [object Object][object Object]x more impactful?
  What alternatives were dismissed too quickly? What competitive or market risks are
  unaddressed? What scope decisions will look foolish in 6 months? Be adversarial.
  No compliments. Just the strategic blind spots.
  File: <plan_path>" -C "$_REPO_ROOT" -s read-only --enable web_search_cached < /dev/null
  _CODEX_EXIT=$?
  if [ "$_CODEX_EXIT" = "[object Object]24" ]; then
    _gstack_codex_log_event "codex_timeout" "6[object Object][object Object]"
    _gstack_codex_log_hang "autoplan" "[object Object]"
    echo "[codex stalled past [object Object][object Object] minutes — tagging as [codex-unavailable] for this phase and proceeding with Claude subagent only]"
  fi
  ```
  Timeout: [object Object][object Object] minutes (shell-wrapper) + [object Object]2 minutes (Bash outer gate). On hang, auto-degrades this phase's Codex voice.

  **Claude CEO subagent** (via Agent tool):
  "Read the plan file at <plan_path>. You are an independent CEO/strategist
  reviewing this plan. You have NOT seen any prior review. Evaluate:
  [object Object]. Is this the right problem to solve? Could a reframing yield [object Object][object Object]x impact?
...
Step [object Object] ([object Object]A-[object Object]F) — run each sub-step and produce:
- [object Object]A: Premise challenge with specific premises named and evaluated
- [object Object]B: Existing code leverage map (sub-problems → existing code)
- [object Object]C: Dream state diagram (CURRENT → THIS PLAN → [object Object]2-MONTH IDEAL)
- [object Object]C-bis: Implementation alternatives table (2-3 approaches with effort/risk/pros/cons)
- [object Object]D: Mode-specific analysis with scope decisions logged
- [object Object]E: Temporal interrogation (HOUR [object Object] → HOUR 6+)
- [object Object]F: Mode selection confirmation

Step [object Object].5 (Dual Voices): Run Claude subagent (foreground Agent tool) first, then
...
Sections [object Object]-[object Object][object Object] — for EACH section, run the evaluation criteria from the loaded skill file:
- Sections WITH findings: full analysis, auto-decide each issue, log to audit trail
- Sections with NO findings: [object Object]-2 sentences stating what was examined and why nothing
  was flagged. NEVER compress a section to just its name in a table row.
- Section [object Object][object Object] (Design): run only if UI scope was detected in Phase [object Object]

**Mandatory outputs from Phase [object Object]:**
...
- Dream state delta (where this plan leaves us vs [object Object]2-month ideal)
- Completion Summary (the full summary table from the CEO skill)

**PHASE [object Object] COMPLETE.** Emit phase-transition summary:
> **Phase [object Object] complete.** Codex: [N concerns]. Claude subagent: [N issues].
> Consensus: [X/6 confirmed, Y disagreements → surfaced at gate].
> Passing to Phase 2.

Do NOT begin Phase 2 until all Phase [object Object] outputs are written to the plan file
...
**Override rules:**
- Focus areas: all relevant dimensions (P[object Object])
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Claude design subagent** (via Agent tool):
  ...
  Error handling: same as Phase [object Object] (both foreground/blocking, degradation matrix applies).
...
[object Object]. Step [object Object] (Design Scope): Rate completeness [object Object]-[object Object][object Object]. Check DESIGN.md. Map existing patterns.

2. Step [object Object].5 (Dual Voices): Run Claude subagent (foreground) first, then Codex. Present under
...
3. Passes [object Object]-7: Run each from loaded skill. Rate [object Object]-[object Object][object Object]. Auto-decide each issue.
...
- [ ] All Phase [object Object] items above confirmed
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Claude eng subagent** (via Agent tool):
  ...
  reviewing this plan. You have NOT seen any prior review. Evaluate:
  [object Object]. Architecture: Is the component structure sound? Coupling concerns?
  2. Edge cases: What breaks under [object Object][object Object]x load? What's the nil/empty/error path?
...
- Evals: always include all relevant suites (P[object Object])
- Test plan: generate artifact at `~/.gstack/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
- TODOS.md: collect all deferred scope expansions from Phase [object Object], auto-write

**Required execution checklist (Eng):**

[object Object]. Step [object Object] (Scope Challenge): Read actual code referenced by the plan. Map each
   sub-problem to existing code. Run the complexity check. Produce concrete findings.

2. Step [object Object].5 (Dual Voices): Run Claude subagent (foreground) first, then Codex. Present
...
═══════════════════════════════════════════════════════════════
  Dimension                           Claude  Codex  Consensus
  ──────────────────────────────────── ─────── ─────── ─────────
  [object Object]. Architecture sound?               —       —      —
...
3. Section [object Object] (Architecture): Produce ASCII dependency graph showing new components
...
6. Section 4 (Performance): Evaluate N+[object Object] queries, memory, caching, slow paths.

**Mandatory outputs from Phase 3:**
- "NOT in scope" section
- "What already exists" section
- Architecture ASCII diagram (Section [object Object])
...
**Skip condition:** If DX scope was NOT detected in Phase [object Object], skip this phase entirely.
...
- Competitive benchmark: run searches if WebSearch available, use reference benchmarks otherwise (P[object Object])
...
- Error message quality: always require problem + cause + fix (P[object Object], completeness)
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex DX voice** (via Bash):
  ...
  [object Object]. Time to hello world: how many steps from zero to working? Target is under 5 minutes.
  ...
  **Claude DX subagent** (via Agent tool):
  ...
  reviewing this plan. You have NOT seen any prior review. Evaluate:
  [object Object]. Getting started: how many steps from zero to hello world? What's the TTHW?
...
[object Object]. Step [object Object] (DX Scope Assessment): Auto-detect product type. Map the developer journey.
   Rate initial DX completeness [object Object]-[object Object][object Object]. Assess TTHW.
...
═══════════════════════════════════════════════════════════════
  Dimension                           Claude  Codex  Consensus
  ──────────────────────────────────── ─────── ─────── ─────────
  [object Object]. Getting started < 5 min?          —       —      —
...
3. Passes [object Object]-8: Run each from loaded skill. Rate [object Object]-[object Object][object Object]. Auto-decide each issue.
...
> **Phase 3.5 complete.** DX overall: [N]/[object Object][object Object]. TTHW: [N] min → [target] min.
...
**Phase [object Object] (CEO) outputs:**
...
AGGREGATED_TASKS=""
if command -v jq >/dev/null 2>&[object Object]; then
  ...
  # Sort by priority (P[object Object] > P2 > P3) then by phase order.
  ...
  for phase in ceo-review design-review eng-review devex-review; do
    ...
    done < <(find "$TASKS_DIR" -maxdepth [object Object] -name "tasks-$phase-*.jsonl" 2>/dev/null | sort)
  ...
  AGGREGATED_TASKS=$(jq -s \
    'group_by([.component, (.files | sort), .title])
     | map(
         ...
         sort_by({P[object Object]:[object Object],P2:[object Object],P3:2}[.priority] // 99, {"ceo-review":[object Object],"design-review":[object Object],"eng-review":2,"devex-review":3}[.phase] // 99) | .[[object Object]]
       )
     | sort_by({P[object Object]:[object Object],P2:[object Object],P3:2}[.priority] // 99, {"ceo-review":[object Object],"design-review":[object Object],"eng-review":2,"devex-review":3}[.phase] // 99)
     | if length == [object Object] then "_No actionable tasks emitted from any phase._" else
...
### Plan Summary
[[object Object]-3 sentence summary]
...
I recommend [X] — [principle]. But [Y] is also viable:
  [[object Object]-sentence downstream impact if you pick Y]
...
**Theme: [topic]** — flagged in [Phase [object Object], Phase 3]. High-confidence signal.
...
- [object Object] user challenges: skip "User Challenges" section
- [object Object] taste decisions: skip "Your Choices" section
- [object Object]-7 taste decisions: flat list
...
- D: make changes, re-run affected phases (scope→[object Object]B, design→2, test plan→3, arch→3). Max 3 cycles.
- E: start over
...
~/.grok/skills/gstack/bin/gstack-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.grok/skills/gstack/bin/gstack-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.grok/skills/gstack/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
~/.grok/skills/gstack/bin/gstack-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
~/.grok/skills/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.grok/skills/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
~/.grok/skills/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
~/.grok/skills/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
- **Two gates.** The non-auto-decided AskUserQuestions are: ([object Object]) premise confirmation in Phase [object Object], and (2) User Challenges — when both models agree the user's stated direction should change. Everything else is auto-decided using the 6 principles.
- **Log every decision.** No silent auto-decisions. Every choice gets a row in the audit trail.
- **Full depth means full depth.** Do not compress or skip sections from the loaded skill files (except the skip list in Phase [object Object]). "Full depth" means: read the code the section asks you to read, produce the outputs the section requires, identify every issue, and decide each one. A one-sentence summary of a section is not "full depth" — it is a skip. If you catch yourself writing fewer than 3 sentences for any review section, you are likely compressing.
