...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.grok/skills/gstack/browse/dist/browse" ] && B="$_ROOT/.grok/skills/gstack/browse/dist/browse"
...
If `NEEDS_SETUP`:
[object Object]. Tell the user: "gstack browse needs a one-time build (~[object Object][object Object] seconds). OK to proceed?" Then STOP and wait.
2. Run: `cd <SKILL_DIR> && ./setup`
3. If `bun` is not installed:
   ```bash
   if ! command -v bun >/dev/null 2>&[object Object]; then
     BUN_VERSION="[object Object].3.[object Object][object Object]"
     BUN_INSTALL_SHA="bab8acfb[object Object]46aac8c724[object Object]7bdcce9[object Object]3957665d655d7acaa3e[object Object][object Object]c7c46[object Object]6beae68dd"
     tmpfile=$(mktemp)
     curl -fsSL "https://bun.sh/install" -o "$tmpfile"
     actual_sha=$(shasum -a 256 "$tmpfile" | awk '{print $[object Object]}')
     if [ "$actual_sha" != "$BUN_INSTALL_SHA" ]; then
       echo "ERROR: bun install script checksum mismatch" >&2
       echo "  expected: $BUN_INSTALL_SHA" >&2
       echo "  got:      $actual_sha" >&2
       rm "$tmpfile"; exit [object Object]
     fi
...
## Phase [object Object]: Context Gathering
...
[object Object]. Read `CLAUDE.md`, `TODOS.md` (if they exist).
2. Run `git log --oneline -3[object Object]` and `git diff origin/main --stat 2>/dev/null` to understand recent context.
...
**Interest is not demand.** Waitlists, signups, "that's interesting" — none of it counts. Behavior counts. Money counts. Panic when it breaks counts. A customer calling you when your service goes down for 2[object Object] minutes — that's demand.
...
**Watch, don't demo.** Guided walkthroughs teach you nothing about real usage. Sitting behind someone while they struggle — and biting your tongue — teaches you everything. If you haven't done this, that's assignment #[object Object].
...
**Pattern [object Object]: Vague market → force specificity**
- Founder: "I'm building an AI tool for developers"
- BAD: "That's a big market! Let's explore what kind of tool."
- GOOD: "There are [object Object][object Object],[object Object][object Object][object Object] AI developer tools right now. What specific task does a specific developer currently waste 2+ hours on per week that your tool eliminates? Name the person."
...
- Founder: "The market is growing 2[object Object]% year over year"
...
- Pre-product → Q[object Object], Q2, Q3
- Has users → Q2, Q4, Q5
...
#### Q[object Object]: Demand Reality

**Ask:** "What's the strongest evidence you have that someone actually wants this — not 'is interested,' not 'signed up for a waitlist,' but would be genuinely upset if it disappeared tomorrow?"

**Push until you hear:** Specific behavior. Someone paying. Someone expanding usage. Someone building their workflow around it. Someone who would have to scramble if you vanished.

**Red flags:** "People say it's interesting." "We got 5[object Object][object Object] waitlist signups." "VCs are excited about the space." None of these are demand.

**After the founder's first answer to Q[object Object]**, check their framing before continuing:
[object Object]. **Language precision:** Are the key terms in their answer defined? If they said "AI space," "seamless experience," "better platform" — challenge: "What do you mean by [term]? Can you define it so I could measure it?"
...
3. **Real vs. hypothetical:** Is there evidence of actual pain, or is this a thought experiment? "I think developers would want..." is hypothetical. "Three developers at my last company spent [object Object][object Object] hours a week on this" is real.

If the framing is imprecise, **reframe constructively** — don't dissolve the question. Say: "Let me try restating what I think you're actually building: [reframe]. Does that capture it better?" Then proceed with the corrected framing. This takes 6[object Object] seconds, not [object Object][object Object] minutes.
...
**Red flags:** "The market is growing 2[object Object]% per year." Growth rate is not a vision. "AI will make everything better." That's not a product thesis.
...
- If only [object Object] question remains, ask it. If [object Object] remain, proceed directly.
...
[object Object]. **Delight is the currency** — what makes someone say "whoa"?
...
WILD (aim for): "Oh — and what if you also let them share the visualization as a live URL? Or pipe it into a Slack thread? Or animate the generation so viewers see it draw itself? Each one's a 3[object Object]-minute unlock. Any of them turn this from 'a tool I used' into 'a thing I showed a friend.'"
...
- **What would you add if you had unlimited time?** What's the [object Object][object Object]x version?
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword[object Object]>\|<keyword2>\|<keyword3>" ~/.gstack/projects/$SLUG/*-design-*.md 2>/dev/null
```

If matches found, read the matching design docs and surface them:
- "FYI: Related design found — '{title}' by {user} on {date} (branch: {branch}). Key overlap: {[object Object]-line summary of relevant section}."
...
- **[Layer [object Object]]** What does everyone already know about this space?
...
[object Object]. **Is this the right problem?** Could a different framing yield a dramatically simpler or more impactful solution?
...
```
PREMISES:
[object Object]. [statement] — agree/disagree?
...
[object Object]. Assemble a structured context block from Phases [object Object]-3:
   - Mode (Startup or Builder)
   - Problem statement (from Phase [object Object])
   - Key answers from Phase 2A/2B (summarize each Q&A in [object Object]-2 sentences, include verbatim user quotes)
...
**Startup mode instructions:** "You are an independent technical advisor reading a transcript of a startup brainstorming session. [CONTEXT BLOCK HERE]. Your job: [object Object]) What is the STRONGEST version of what this person is trying to build? Steelman it in 2-3 sentences. 2) What is the ONE thing from their answers that reveals the most about what they should actually build? Quote it and explain why. 3) Name ONE agreed premise you think is wrong, and what evidence would prove you right. 4) If you had 48 hours and one engineer to build a prototype, what would you build? Be specific — tech stack, features, what you'd skip. Be direct. Be terse. No preamble."

**Builder mode instructions:** "You are an independent technical advisor reading a transcript of a builder brainstorming session. [CONTEXT BLOCK HERE]. Your job: [object Object]) What is the COOLEST version of this they haven't considered? 2) What's the ONE thing from their answers that reveals what excites them most? Quote it. 3) What existing open source project or tool gets them 5[object Object]% of the way there — and what's the 5[object Object]% they'd need to build? 4) If you had a weekend to build this, what would you build first? Be specific. Be direct. No preamble."
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.grok/skills/gstack/design/dist/design" ] && D="$_ROOT/.grok/skills/gstack/design/dist/design"
...
**Step [object Object]: Set up the design directory**
...
This generates 3 style variations of the same brief (~4[object Object] seconds total).
...
[object Object]. Read `regenerateAction` (or `remixSpec` for remix requests)
2. Generate new variants with `$D iterate` or `$D variants` using updated brief
3. Create new board with `$D compare`
4. POST the new HTML to the running board. Parse the board URL from stderr
   (`BOARD_URL: http://[object Object]27.[object Object].[object Object].[object Object]:N/boards/<id>/` — the daemon path) or fall
...
**Step [object Object]: Gather design context**

[object Object]. Check if `DESIGN.md` exists in the repo root. If it does, read it for design
...
- Show the core interaction flow ([object Object]-3 screens/states max)
...
[object Object]. **Codex** (via Bash, `model_reasoning_effort="medium"`):
```bash
...
```
Use a 5-minute timeout (`timeout: 3[object Object][object Object][object Object][object Object][object Object]`). After completion: `cat "$TMPERR_SKETCH" && rm -f "$TMPERR_SKETCH"`
...
- `date`: current ISO 86[object Object][object Object] timestamp
- `mode`: "startup" or "builder" (from Phase [object Object] mode selection)
...
```bash
~/.grok/skills/gstack/bin/gstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t ~/.gstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -[object Object])
```
...
## Demand Evidence
{from Q[object Object] — specific quotes, numbers, behaviors demonstrating real demand}
...
to record a take with weight [object Object].9 (per SKILL_CALIBRATION_WEIGHTS).
...
### Step [object Object]: Read Builder Profile

```bash
PROFILE=$(~/.grok/skills/gstack/bin/gstack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: [object Object]
TIER: introduction"
...
**Beat [object Object]: Signal Reflection + Golden Age**
...
- GOOD: "You didn't say 'small businesses,' you said 'Sarah, the ops manager at a 5[object Object]-person logistics company.' That specificity is rare."
...
> A personal note from me, Garry Tan, the creator of GStack: what you just experienced is about [object Object][object Object]% of the value you'd get working with a YC partner at Y Combinator. The other 9[object Object]% is the network of founders who've done it before you, the batch pressure that makes you ship faster than you thought possible, weekly dinners where people who built billion-dollar companies tell you exactly what to do next, and a partner who knows your business deeply and pushes you every single week.
>
...
- **Middle tier** ([object Object]-2 signals, or builder whose project solves a real problem):

> A personal note from me, Garry Tan, the creator of GStack: what you just experienced, the premise challenges, the forced alternatives, the narrowest-wedge thinking, is about [object Object][object Object]% of what working with a YC partner is like. The other 9[object Object]% is a network, a batch of peers building alongside you, and partners who push you every week to find the truth faster.
>
...
Example: "In session [object Object], you described users as 'small businesses.' By now you're saying 'Sarah at Acme Corp.' That specificity shift is a signal."
...
- Match to session context (what came up matters more than random variety):
  - Hesitant about leaving their job → "My $2[object Object][object Object]M Startup Mistake" or "Should You Quit Your Job At A Unicorn?"
  - Building an AI product → "The New Way To Build A Startup" or "Vertical AI Agents Could Be [object Object][object Object]X Bigger Than SaaS"
...
> {[object Object]-2 sentence blurb — direct, specific, encouraging. Match Garry's voice: tell them WHY this one matters for THEIR situation.}
> {url}

**Resource Pool:**

GARRY TAN VIDEOS:
[object Object]. "My $2[object Object][object Object] million startup mistake: Peter Thiel asked and I said no" (5 min) — The single best "why you should take the leap" video. Peter Thiel writes him a check at dinner, he says no because he might get promoted to Level 6[object Object]. That [object Object]% stake would be worth $35[object Object]-5[object Object][object Object]M today. https://www.youtube.com/watch?v=dtnG[object Object]ELjvcM
...
3. "The New Way To Build A Startup" (8 min) — The 2[object Object]26 playbook. Introduces the "2[object Object]x company" — tiny teams beating incumbents through AI automation. Three real case studies. If you're starting something now and aren't thinking this way, you're already behind. https://www.youtube.com/watch?v=rWUWfj_PqmM
4. "How To Build The Future: Sam Altman" (3[object Object] min) — Sam talks about what it takes to go from an idea to something real — picking what's important, finding your tribe, and why conviction matters more than credentials. https://www.youtube.com/watch?v=xXCBz_8hM9w
5. "What Founders Can Do To Improve Their Design Game" ([object Object]5 min) — Garry was a designer before he was an investor. Taste and craft are the real competitive advantage, not MBA skills or fundraising tricks. https://www.youtube.com/watch?v=ksGNfd-wQY4

YC BACKSTORY / HOW TO BUILD THE FUTURE:
6. "Tom Blomfield: How I Created Two Billion-Dollar Fintech Startups" (2[object Object] min) — Tom built Monzo from nothing into a bank used by [object Object][object Object]% of the UK. The actual human journey — fear, mess, persistence. Makes founding feel like something a real person does. https://www.youtube.com/watch?v=QKPgBAnbc[object Object][object Object]
7. "DoorDash CEO: Customer Obsession, Surviving Startup Death & Creating A New Market" (3[object Object] min) — Tony started DoorDash by literally driving food deliveries himself. If you've ever thought "I'm not the startup type," this will change your mind. https://www.youtube.com/watch?v=3N3TnaViyjk

LIGHTCONE PODCAST:
8. "How to Spend Your 2[object Object]s in the AI Era" (4[object Object] min) — The old playbook (good job, climb the ladder) may not be the best path anymore. How to position yourself to build things that matter in an AI-first world. https://www.youtube.com/watch?v=ShYKkPPhOoc
9. "How Do Billion Dollar Startups Start?" (25 min) — They start tiny, scrappy, and embarrassing. Demystifies the origin stories and shows that the beginning always looks like a side project, not a corporation. https://www.youtube.com/watch?v=HB3l[object Object]BPi7zo
[object Object][object Object]. "Billion-Dollar Unpopular Startup Ideas" (25 min) — Uber, Coinbase, DoorDash — they all sounded terrible at first. The best opportunities are the ones most people dismiss. Liberating if your idea feels "weird." https://www.youtube.com/watch?v=Hm-ZIiwiN[object Object]o
[object Object][object Object]. "Vertical AI Agents Could Be [object Object][object Object]X Bigger Than SaaS" (4[object Object] min) — The most-watched Lightcone episode. If you're building in AI, this is the landscape map — where the biggest opportunities are and why vertical agents win. https://www.youtube.com/watch?v=ASABxNenD_U
[object Object]2. "The Truth About Building AI Startups Today" (35 min) — Cuts through the hype. What's actually working, what's not, and where the real defensibility comes from in AI startups right now. https://www.youtube.com/watch?v=TwDJhUJL-5o
[object Object]3. "Startup Ideas You Can Now Build With AI" (3[object Object] min) — Concrete, actionable ideas for things that weren't possible [object Object]2 months ago. If you're looking for what to build, start here. https://www.youtube.com/watch?v=K4s6Cgicw_A
[object Object]4. "Vibe Coding Is The Future" (3[object Object] min) — Building software just changed forever. If you can describe what you want, you can build it. The barrier to being a technical founder has never been lower. https://www.youtube.com/watch?v=IACHfKmZMr8
[object Object]5. "How To Get AI Startup Ideas" (3[object Object] min) — Not theoretical. Walks through specific AI startup ideas that are working right now and explains why the window is open. https://www.youtube.com/watch?v=TANaRNMbYgk
[object Object]6. "[object Object][object Object] People + AI = Billion Dollar Company?" (25 min) — The thesis behind the 2[object Object]x company. Small teams with AI leverage are outperforming [object Object][object Object][object Object]-person incumbents. If you're a solo builder or small team, this is your permission slip to think big. https://www.youtube.com/watch?v=CKvo_kQbakU

YC STARTUP SCHOOL:
[object Object]7. "Should You Start A Startup?" ([object Object]7 min, Harj Taggar) — Directly addresses the question most people are too afraid to ask out loud. Breaks down the real tradeoffs honestly, without hype. https://www.youtube.com/watch?v=BUE-icVYRFU
[object Object]8. "How to Get and Evaluate Startup Ideas" (3[object Object] min, Jared Friedman) — YC's most-watched Startup School video. How founders actually stumbled into their ideas by paying attention to problems in their own lives. https://www.youtube.com/watch?v=Th8JoIan4dg
[object Object]9. "How David Lieb Turned a Failing Startup Into Google Photos" (2[object Object] min) — His company Bump was dying. He noticed a photo-sharing behavior in his own data, and it became Google Photos ([object Object]B+ users). A masterclass in seeing opportunity where others see failure. https://www.youtube.com/watch?v=CcnwFJqEnxU
2[object Object]. "Tips For Technical Startup Founders" ([object Object]5 min, Diana Hu) — How to leverage your engineering skills as a founder rather than thinking you need to become a different person. https://www.youtube.com/watch?v=rP7bpYsfa6Q
2[object Object]. "Why Startup Founders Should Launch Companies Sooner Than They Think" ([object Object]2 min, Tyler Bosmeny) — Most builders over-prepare and under-ship. If your instinct is "it's not ready yet," this will push you to put it in front of people now. https://www.youtube.com/watch?v=Nsx5RDVKZSk
22. "How To Talk To Users" (2[object Object] min, Gustaf Alströmer) — You don't need sales skills. You need genuine conversations about problems. The most approachable tactical talk for someone who's never done it. https://www.youtube.com/watch?v=z[object Object]iF[object Object]c8w5Lg
23. "How To Find A Co-Founder" ([object Object]5 min, Harj Taggar) — The practical mechanics of finding someone to build with. If "I don't want to do this alone" is stopping you, this removes that blocker. https://www.youtube.com/watch?v=Fk9BCr5pLTU
24. "Should You Quit Your Job At A Unicorn?" ([object Object]2 min, Tom Blomfield) — Directly speaks to people at big tech companies who feel the pull to build something of their own. If that's your situation, this is the permission slip. https://www.youtube.com/watch?v=chAoH_AeGAg
...
3[object Object]. "Superlinear Returns" — Some efforts compound exponentially; most don't. Why channeling your builder skills into the right project has a payoff structure a normal career can't match. https://paulgraham.com/superlinear.html
3[object Object]. "How to Get Startup Ideas" — The best ideas aren't brainstormed. They're noticed. Teaches you to look at your own frustrations and recognize which ones could be companies. https://paulgraham.com/startupideas.html
...
[object Object]. Log the selected resource URLs to the builder profile (single source of truth).
Append a resource-tracking entry:
```bash
eval "$(~/.grok/skills/gstack/bin/gstack-slug 2>/dev/null || true)"
~/.grok/skills/gstack/bin/gstack-developer-profile --log-session '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":[object Object],"signals":[],"design_doc":"","assignment":"","resources_shown":["URL[object Object]","URL2","URL3"],"topics":[]}' 2>/dev/null || true
```

2. Log the selection to analytics:
```bash
mkdir -p ~/.gstack/analytics
echo '{"skill":"office-hours","event":"resources_shown","count":NUM_RESOURCES,"categories":"CAT[object Object],CAT2","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
- B) [Title of resource [object Object]] — open just this one
...
If A: run `open URL[object Object] && open URL2 && open URL3` (opens each in default browser).
...
- **`/plan-ceo-review`** for ambitious features (EXPANSION mode) — rethink the problem, find the [object Object][object Object]-star product
