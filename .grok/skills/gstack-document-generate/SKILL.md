...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"document-generate","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
[object Object]. **Standalone** — the user points you at a feature, module, or project and says "document this"
...
## Step [object Object]: Scope & Intent

[object Object]. Determine what to document:
...
## Step [object Object]: Codebase Archaeology (Research Phase)
...
[object Object]. **Map the project structure:**

```bash
find . -type f -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.gstack/*" -not -path "./dist/*" -not -path "./build/*" -not -path "./.next/*" | head -2[object Object][object Object]
```
...
[object Object]. [Action verb] [specific instruction]
...
## Step [object Object]: [Set up the foundation]
...
[object Object]. **Add cross-links between quadrants.** Every reference doc should link to its how-to.
...
**Completeness gate:**
- [ ] Reference docs cover [object Object][object Object][object Object]% of public surface
...
[object Object]. Stage new documentation files by name (never `git add -A` or `git add .`).
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  ~/.grok/skills/gstack/bin/gstack-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
...
- **Research before writing.** Step [object Object] is not optional. Read the code, read the tests, read the
