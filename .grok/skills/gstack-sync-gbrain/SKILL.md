...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"sync-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
**Architecture (post-codex review):** This skill uses gbrain v[object Object].2[object Object].[object Object]+'s
...
- `/sync-gbrain` — incremental sync (default; mtime fast-path; ~5[object Object]ms steady-state)
...
- `/sync-gbrain --refresh-cache` — force-rebuild brain-aware planning cache (v[object Object].48; replaces /brain-refresh-context per D[object Object] fold). Skips code + memory stages; routes to `gstack-brain-cache refresh --project <slug>`.
- `/sync-gbrain --audit` — emit summary of gstack-owned pages per project + sensitive-content audit (v[object Object].48 / D[object Object][object Object] lifecycle). Read-only.
...
the SALIENCE_DEFAULT_ALLOWLIST (T[object Object]7 / D9 leak check). Read-only; no
modifications to brain or cache.

---

## Step [object Object]: State probe

Before doing anything, check that /setup-gbrain has been run on this Mac.

```bash
~/.grok/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null
```

**Brain trust policy gate (v[object Object].48 / Phase [object Object].5 / D4 — added by T[object Object]3+T5c):**
...
**Split-engine model (v[object Object].34.[object Object].[object Object]+).** Code stage runs locally against the
...
the brain admin's pull job (plan D[object Object][object Object]). Brain-sync (the `gstack-brain-sync`
...
## Step [object Object].5: Local engine pre-flight (plan D[object Object]2)

Read `gbrain_local_status` from the Step [object Object] detect output. Branch as follows
...
- **`missing-config`** AND `gbrain_mcp_mode == "remote-http"`: tell the user
  ...
  `gbrain init --pglite --json --embedding-model voyage:voyage-code-3 --embedding-dimensions [object Object][object Object]24`
  ...
  `runMemoryIngest()` will return SKIP per plan D[object Object]2; only `runBrainSyncPush()`
...
- **`broken-config`** OR **`broken-db`**: STOP with a clear message:
  ...
  engine (status: {gbrain_local_status}). Two options:
    [object Object]. Re-run /setup-gbrain — Step [object Object].5 offers Retry / Switch to PGLite /
    ...
    2. Repair manually: mv ~/.gbrain/config.json ~/.gbrain/config.json.bak
       && gbrain init --pglite --json --embedding-model voyage:voyage-code-3 \
          --embedding-dimensions [object Object][object Object]24   (drop voyage flags if VOYAGE_API_KEY unset)
...
This pre-flight short-circuits the orchestrator before it spends ~8[object Object]ms
probing the engine again. The orchestrator independently runs the same
classifier for defense-in-depth, but Step [object Object].5's STOP is where the user
...
```bash
bun run ~/.grok/skills/gstack/bin/gstack-gbrain-sync.ts <user-args>
```
...
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' ~/.gstack/.gbrain-sync-state.json 2>/dev/null \
  | head -[object Object] | sed 's/.*"source_id":"//;s/".*//')
...
If `PAGES` is [object Object] or empty AND the user did NOT pass `--no-code` AND mode was
not `--full`, AskUserQuestion via the format in the preamble:

> D[object Object] — This repo has [object Object] indexed pages in gbrain. Run a full code reindex now?
>
> ELI[object Object][object Object]: gbrain hasn't indexed this repo's code yet. The semantic search
...
`count: [object Object]` until a `gbrain dream` cycle runs the `resolve_symbol_edges` phase for
...
capability only at cycle runtime (no pre-flight query as of [object Object].4[object Object].x), so we can't
...
`PAGES > [object Object]`, AskUserQuestion via the format in the preamble:

> D2 — This repo's call graph isn't built. Build it now?
>
> ELI[object Object][object Object]: `gbrain code-callers`/`code-callees` (who calls this function / what it
...
>
> Recommendation: A — call-graph queries return [object Object] until this runs, and the code
...
embedding key, or [object Object] edges matched). Do not claim success on a WARN.
...
if [ -f ~/.gbrain/config.json ] && \
  ...
  # Without it, search silently returns no results (#[object Object]435).
  export GBRAIN_PREPARE=true
  if echo "ping" | gbrain put "$SLUG" >/dev/null 2>&[object Object]; then
    # Retry search up to 3 times with [object Object]s delay — under transaction-mode
    # pooling the search index may not be visible on the next connection
    # immediately after the put.
    for _attempt in [object Object] 2 3; do
...
**If `CAPABILITY_OK=[object Object]`** — write or update the block. Idempotent: find the
...
no `--source` flag needed (gbrain >= [object Object].4[object Object].38.[object Object]; on older gbrain the call-graph
...
`count: [object Object]`. This only works if this source's gbrain schema pack extracts code
...
to avoid racing it (#[object Object]734). Prefer registering user repos with `gbrain sources
...
**If `CAPABILITY_OK=[object Object]`** — REMOVE the block entirely if present. Use the same
...
Print a status block matching `/setup-gbrain` Step [object Object][object Object] conventions. Each row
...
[object Object]. **If a dream stage ran this invocation** (`--dream`, or `--full` auto-build),
   ...
   - `WARN dream ran but resolved [object Object] edges (no code symbols matched yet)`
2. **Otherwise** fall back to the `CYCLE` value from Step 3.5, with honest wording
   ...
   - `unknown` → `WARN could not probe call graph (doctor unavailable) — run /sync-gbrain --dream if code-callers returns [object Object]`
