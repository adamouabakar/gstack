...
description: |
  ...
  3[object Object]KB overhead, zero deps. Smart API routing: picks the right Pretext patterns
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-html","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
## Step [object Object]: Input Detection
...
_CEO=$(ls -t ~/.gstack/projects/$SLUG/ceo-plans/*.md 2>/dev/null | head -[object Object])
[ -n "$_CEO" ] && echo "CEO_PLAN: $_CEO" || echo "NO_CEO_PLAN"
```

```bash
setopt +o nomatch 2>/dev/null || true
_APPROVED=$(ls -t ~/.gstack/projects/$SLUG/designs/*/approved.json 2>/dev/null | head -[object Object])
[ -n "$_APPROVED" ] && echo "APPROVED: $_APPROVED" || echo "NO_APPROVED"
```

```bash
setopt +o nomatch 2>/dev/null || true
_VARIANTS=$(ls -t ~/.gstack/projects/$SLUG/designs/*/variant-*.png 2>/dev/null | head -[object Object])
[ -n "$_VARIANTS" ] && echo "VARIANTS: $_VARIANTS" || echo "NO_VARIANTS"
```

```bash
setopt +o nomatch 2>/dev/null || true
_FINALIZED=$(ls -t ~/.gstack/projects/$SLUG/designs/*/finalized.html 2>/dev/null | head -[object Object])
...
If fresh or no finalized.html: proceed to Step [object Object] with the approved PNG as the
visual reference.
...
If B: proceed to Step [object Object] in "plan-driven mode." There is no approved PNG, the plan is
...
If D: proceed to Step [object Object] in "freeform mode." Ask the user for a screen name.
...
## Step [object Object]: Design Analysis

[object Object]. If `$D` is available (`DESIGN_READY`), extract a structured implementation spec:
```bash
...
```bash
[ -f package.json ] && cat package.json | grep -o '"react"\|"svelte"\|"vue"\|"@angular/core"\|"solid-js"\|"preact"' | head -[object Object] || echo "NONE"
```
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.grok/skills/gstack/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/.grok/skills/gstack/design-html/vendor/pretext.js"
[ -z "$_PRETEXT_VENDOR" ] && [ -f ~/.grok/skills/gstack/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=~/.grok/skills/gstack/design-html/vendor/pretext.js
...
- CSS custom properties for design tokens from DESIGN.md / Step [object Object] extraction
...
- Breakpoint-specific adjustments at 375px, 768px, [object Object][object Object]24px, [object Object]44[object Object]px
...
**Pattern [object Object]: Basic height computation (Simple layout, Card/grid)**
```js
import { prepare, layout } from './pretext-inline.js'
// Or if inlined: const { prepare, layout } = window.Pretext

// [object Object]. PREPARE — one-time, after fonts load
...
function shrinkwrap(text, font, maxWidth, lineHeight) {
  ...
  let lo = [object Object], hi = maxWidth
  while (hi - lo > [object Object]) {
    const mid = (lo + hi) / 2
...
function layoutAroundObstacles(text, font, containerWidth, lineHeight, obstacles) {
  ...
  while (true) {
    ...
    lines.push({ text: result.text, width: result.width, x: [object Object], y })
    state = result.state
    y += lineHeight
  }
...
prepare(text, font) → handle
  ...
  Font: CSS shorthand like '[object Object]6px Inter' or 'bold 24px Georgia'.
...
cd "$_OUTPUT_DIR"
python3 -m http.server [object Object] --bind [object Object]27.[object Object].[object Object].[object Object] &
_SERVER_PID=$!
_PORT=$(lsof -i -P -n | grep "$_SERVER_PID" | grep LISTEN | awk '{print $9}' | cut -d: -f2 | head -[object Object])
...
$B screenshot /tmp/gstack-verify-desktop.png --width [object Object]44[object Object]
```
...
```
LOOP:
  [object Object]. If server is running, tell user to open http://localhost:PORT/finalized.html
...
Maximum [object Object][object Object] iterations. If the user hasn't said "done" after [object Object][object Object], use AskUserQuestion:
"We've done [object Object][object Object] rounds of refinement. Want to continue iterating or call it done?"
...
```json
{
  ...
  "date": "<ISO 86[object Object][object Object]>",
  "screen": "<screen name>",
...
- **Source of truth fidelity over code elegance.** When an approved mockup exists,
  pixel-match it. If that requires `width: 3[object Object]2px` instead of a CSS grid class, that's
...
- **Always use Pretext for text layout.** Even if the design looks simple, Pretext
  ensures correct height computation on resize. The overhead is 3[object Object]KB. Every page benefits.
