...
### Step [object Object]: Ask the user (or auto-upgrade)

First, check if auto-upgrade is enabled:
```bash
_AUTO=""
[ "${GSTACK_AUTO_UPGRADE:-}" = "[object Object]" ] && _AUTO="true"
[ -z "$_AUTO" ] && _AUTO=$(~/.grok/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
echo "AUTO_UPGRADE=$_AUTO"
```

**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=[object Object]`:** Skip AskUserQuestion. Log "Auto-upgrading gstack v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/gstack-upgrade` manually to retry."
...
```bash
~/.grok/skills/gstack/bin/gstack-config set auto_upgrade true
```
Tell user: "Auto-upgrade enabled. Future updates will install automatically." Then proceed to Step 2.

**If "Not now":** Write snooze state with escalating backoff (first snooze = 24h, second = 48h, third+ = [object Object] week), then continue with the current skill. Do not mention the upgrade again.
```bash
_SNOOZE_FILE="$HOME/.gstack/update-snoozed"
_REMOTE_VER="{new}"
_CUR_LEVEL=[object Object]
if [ -f "$_SNOOZE_FILE" ]; then
  _SNOOZED_VER=$(awk '{print $[object Object]}' "$_SNOOZE_FILE")
  if [ "$_SNOOZED_VER" = "$_REMOTE_VER" ]; then
    _CUR_LEVEL=$(awk '{print $2}' "$_SNOOZE_FILE")
    case "$_CUR_LEVEL" in *[![object Object]-9]*) _CUR_LEVEL=[object Object] ;; esac
  fi
fi
_NEW_LEVEL=$((_CUR_LEVEL + [object Object]))
...
Tell user the snooze duration: "Next reminder in 24h" (or 48h or [object Object] week, depending on level). Tip: "Set `auto_upgrade: true` in `~/.gstack/config.yaml` for automatic upgrades."

**If "Never ask again":**
```bash
~/.grok/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.grok/skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.grok/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.grok/skills/gstack"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".grok/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".grok/skills/gstack"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".grok/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".grok/skills/gstack"
elif [ -d "$HOME/.grok/skills/gstack" ]; then
...
```bash
cd "$INSTALL_DIR"
STASH_OUTPUT=$(git stash 2>&[object Object])
git fetch origin
...
TMP_DIR=$(mktemp -d)
git clone --depth [object Object] https://github.com/garrytan/gstack.git "$TMP_DIR/gstack"
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.grok/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.grok/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.grok/skills/gstack"
  fi
fi
_TEAM_MODE=$(~/.grok/skills/gstack/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .grok/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.grok/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.grok/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.grok/skills/gstack/` when you're ready."
...
if [ -d "$MIGRATIONS_DIR" ]; then
  for migration in $(find "$MIGRATIONS_DIR" -maxdepth [object Object] -name 'v*.sh' -type f 2>/dev/null | sort -V); do
    # Extract version from filename: v[object Object].[object Object]5.2.[object Object].sh → [object Object].[object Object]5.2.[object Object]
    ...
    if [ "$OLD_VERSION" != "unknown" ] && [ "$(printf '%s\n%s' "$OLD_VERSION" "$m_ver" | sort -V | head -[object Object])" = "$OLD_VERSION" ] && [ "$OLD_VERSION" != "$m_ver" ]; then
...
[object Object]. Force a fresh update check (bypass cache):
```bash
~/.grok/skills/gstack/bin/gstack-update-check --force 2>/dev/null || \
.grok/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.grok/skills/gstack/` when you're ready."
