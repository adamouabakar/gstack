...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"benchmark-models","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
## Step [object Object]: Locate the binary

```bash
BIN="$HOME/.grok/skills/gstack/bin/gstack-model-benchmark"
[ -x "$BIN" ] || BIN=".grok/skills/gstack/bin/gstack-model-benchmark"
[ -x "$BIN" ] || { echo "ERROR: gstack-model-benchmark not found. Run ./setup in the gstack install dir." >&2; exit [object Object]; }
echo "BIN: $BIN"
```
...
## Step [object Object]: Choose a prompt
...
- **Options:**
  - A) Benchmark one of my gstack skills (we'll pick which skill next). Completeness: [object Object][object Object]/[object Object][object Object].
  - B) Use an inline prompt — type it on the next turn. Completeness: 8/[object Object][object Object].
  - C) Point at a prompt file on disk — specify path on the next turn. Completeness: 8/[object Object][object Object].
...
- **Options:**
  - A) All authed providers. Completeness: [object Object][object Object]/[object Object][object Object].
  - B) Only Claude. Completeness: 6/[object Object][object Object] (no cross-model signal — use /ship's review for solo claude benchmarks instead).
  - C) Pick two — specify on next turn. Completeness: 8/[object Object][object Object].
...
- **Simplify:** "The quality judge scores each model's output on a [object Object]-[object Object][object Object] scale using Anthropic's Claude as a tiebreaker. Adds ~$[object Object].[object Object]5/run. Recommended if you care about output quality, not just latency and cost."
- **RECOMMENDATION:** A — the whole point is comparing quality, not just speed.
- **Options:**
  - A) Enable judge (adds ~$[object Object].[object Object]5). Completeness: [object Object][object Object]/[object Object][object Object].
  - B) Skip judge — speed/cost/tokens only. Completeness: 7/[object Object][object Object].
...
Construct the command from Step [object Object], 2, 3 decisions:

```bash
"$BIN" <prompt-spec> --models <picked-models> [--judge] --output table
```

Where `<prompt-spec>` is either `--prompt "<text>"` (Step [object Object]B), a file path (Step [object Object]A or [object Object]C), and `<picked-models>` is the comma-separated list from Step 2.

Stream the output as it arrives. This is slow — each provider runs the prompt fully. Expect 3[object Object]s-5min depending on prompt complexity and whether `--judge` is on.
...
- **Options:**
  - A) Save to `~/.gstack/benchmarks/<date>-<skill-or-prompt-slug>.json`. Completeness: [object Object][object Object]/[object Object][object Object].
  - B) Just print, don't save. Completeness: 5/[object Object][object Object] (loses trend data).
