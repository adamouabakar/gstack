...
description: |
  ...
  the codified script in ~2[object Object][object Object]ms instead of re-driving the page. Walks
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"skillify","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
code so the next `/scrape` call on the same intent runs in ~2[object Object][object Object]ms.
...
## Step [object Object] — Provenance guard (D[object Object])

Walk back through the conversation, **at most [object Object][object Object] agent turns**, looking
...
ELI[object Object][object Object]: Pick a short name we'll use to find this skill next time you say
...
E.g. `fixtures/lobste-rs-2[object Object]26-[object Object]4-27.html`.
...
describe('<name> parser', () => {
  ...
  it('returns at least one item from the bundled fixture', () => {
    expect(items.length).toBeGreaterThan([object Object]);
  });
...
[object Object]. The bundled `hackernews-frontpage` skill — look at its tier path from
...
2. The active gstack skills install at `~/.grok/skills/gstack/`. Read
...
`_lib/browse-client.ts` byte-identical to the canonical. Phase [object Object] decision
...
The SKILL.md content for `<name>` follows the Phase [object Object] frontmatter
contract:
...
source: agent
version: [object Object].[object Object].[object Object]
...
[object Object]. Read the test output. If the failure is a fixable parser bug,
...
ELI[object Object][object Object]: The script ran clean against the snapshot we captured. Saying yes
...
~3[object Object]s of synthesis work.
...
## Step [object Object][object Object] — Commit (atomic) or discard
...
## Step [object Object][object Object] — Confirm + verify
...
/scrape calls matching '<canonical-trigger>' will run in ~2[object Object][object Object]ms."
...
- **Bun runtime required.** The codified skill runs as a Bun process
  (`bun run script.ts`). Phase [object Object] design carry-over (Codex finding #7).
...
- Codify mutating flows (those are /automate's job — Phase 2 P[object Object])
