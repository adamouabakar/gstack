...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
The iOS app's `StateServer` binds loopback only (`::[object Object]` + `[object Object]27.[object Object].[object Object].[object Object]`). Tailnet
...
tokens (default [object Object]h) for remote agents.
...
## Phase [object Object]: Session warm-start (optional)

If `~/.gstack/ios-qa-session.json` exists and the device is still connected,
skip Phase [object Object]-2 and jump to Phase 3. The session cache holds the rotated token,
...
if [ -f "$SESSION" ] && [ "$COLD" != "[object Object]" ]; then
  ...
  if curl -sf "http://[object Object]27.[object Object].[object Object].[object Object]:$CACHED_PORT/healthz" > /dev/null; then
    echo "Warm start: daemon alive, device $CACHED_UDID connected"
  fi
fi
```

## Phase [object Object]: Read source, plan codegen

[object Object]. Walk the app source (passed as `--source <dir>`) and identify all `@Observable`
...
2. Run `swift run --package-path $GSTACK_HOME/ios-qa/scripts/gen-accessors-tool gen-accessors --input <source-dir>`.
   ...
   Subsequent runs are content-hash-cached and finish in ~5[object Object]ms.
...
[object Object]. Add the `DebugBridge` SPM dependency to the app's `Package.swift`. The package
   ...
   - `DebugBridgeTouch` (Objective-C, iOS-only) — KIF-derived in-process touch
     synthesis with iOS [object Object]8+ `_UIHitTestContext` SwiftUI hit-testing.
...
[object Object]. `GET /screenshot` (via daemon) → save PNG.
...
interface (never `[object Object].[object Object].[object Object].[object Object]`). Requires `tailscaled` to be running locally and
...
| `4[object Object]3 identity_not_allowed` from `/auth/mint` | identity missing from allowlist | Run `gstack-ios-qa-mint --remote <identity>` on the Mac |
| `4[object Object]9 schema_mismatch` on `/state/restore` | snapshot from older app build | Discard the snapshot; re-capture |
| `5[object Object]3 device_disconnected` from proxy | USB tunnel dropped | Reconnect device; daemon auto-reconnects within 3[object Object]s |
| `429 rate_limited` from `/auth/mint` | >[object Object][object Object] mints/min from one identity | Wait 6[object Object]s; check audit log for anomalies |
| `4[object Object]3 body_too_large` on `/state/restore` | snapshot >[object Object]MB | Increase `--max-body` or trim snapshot |
