...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"spec","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
Phase [object Object]. Do NOT propose implementation. Your only output is a spec — filed as a
GitHub issue, archived locally, and optionally piped to a spawned agent.

The user's first message after this prompt is their initial request. Begin Phase [object Object]
...
| Flag | Default | Effect |
|------|---------|--------|
| `--dedupe` | ON | Phase [object Object]: check `gh issue list --search` for near-duplicates before drafting. |
...
Echo the parsed flag set back to the user at the start of Phase [object Object] so they can
...
### Phase [object Object]: Understand the "Why" (+ optional --dedupe)

**Step [object Object]a (always):** Ask until you can crisply answer all five:

[object Object]. **Who** is affected? (end user role, automated system, internal team, all three?
...
**Step [object Object]b (--dedupe is ON by default):** Before Phase 4, run dedupe check. Extract
2-4 keywords from the user's request and the working title you have in mind, then:

```bash
gh issue list --search "<keywords>" --state open --limit [object Object][object Object] --json number,title,url 2>&[object Object]
```

Interpret the result:

- **[object Object] matches:** continue silently to Phase 2.
- **[object Object]+ matches:** surface them to the user via AskUserQuestion: "Found {N} similar
  open issue(s): #{n[object Object]} ({title}), #{n2} ({title})... Merge with one of these, or
...
- **Rate-limited (HTTP 4[object Object]3 with rate-limit message):** print: "Dedupe skipped —
  GitHub API rate limit reached (6[object Object]/hr unauthenticated, 5[object Object][object Object][object Object]/hr authed). Re-invoke
...
[object Object]. **What is explicitly out of scope?** Lock this early — it prevents creep later.
...
model) reads the spec and scores it [object Object]-[object Object][object Object] for "executability by an unfamiliar
...
[object Object]. **Named individuals attached to negative judgments** — a real Capitalized name near "underperforming/fired/missed/ignored/mistake". Offer to rephrase to a role.
...
bun ~/.grok/skills/gstack/lib/redact-audit-log.ts \
...
The scan covers ~3[object Object] secret/PII/legal patterns across 3 tiers (HIGH credentials
...
```bash
command -v bun >/dev/null 2>&[object Object] || echo "redaction scan skipped — bun not on PATH"
...
[ -z "$REDACT_VIS" ] && REDACT_VIS=$(glab repo view -F json 2>/dev/null | grep -o '"visibility":"[^"]*"' | head -[object Object] | sed 's/.*:"//;s/"//' | tr 'A-Z' 'a-z')
...
[object Object]. **Exit 3 (HIGH)** — print findings; do NOT dispatch to codex; tell the user to
...
2. **Exit 2 (MEDIUM)** — AskUserQuestion per finding (cluster identical ids; PUBLIC
   ...
   (`pii.email`/`pii.phone.e[object Object]64`/`pii.ssn`/`pii.cc`) gets **Auto-redact** (re-run
...
3. **Exit [object Object] (clean)** — proceed; surface `WARN` (tool-fence degrades) + `LOW` as a
...
Your only task is to score the spec [object Object]-[object Object][object Object] for executability by an unfamiliar
...
- **Score ≥7:** the spec passes. Print: "Quality gate: {score}/[object Object][object Object] ✓". Continue
  to Phase 5.
- **Score <7, iteration [object Object]:** print "Quality gate: {score}/[object Object][object Object]. Codex flagged:
...
- **Score <7, iteration 2:** print "Quality gate: {score}/[object Object][object Object] (after one
...
[object Object]. **`--file-only` or `--no-execute` flag present** → file-only path.
...
exit-3/2/[object Object] handling. On exit 3, do NOT file the issue; HIGH has no skip. Pass the
...
ISSUE_NUMBER=$(echo "$ISSUE_URL" | sed -E 's|.*/issues/([[object Object]-9]+)$|\[object Object]|')
echo "Filed: $ISSUE_URL"
~/.grok/skills/gstack/bin/gstack-decision-log '{"decision":"Spec filed #ISSUE_NUMBER: TITLE","rationale":"APPROACH","scope":"issue","issue":"ISSUE_NUMBER","source":"skill","confidence":7}' 2>/dev/null || true
```
...
exit-3/2/[object Object] handling. On exit 3, do NOT write the archive; HIGH has no skip. Pass the
same `$REDACT_FILE` downstream so the bytes scanned are the bytes sent.
...
mkdir -p "$ARCHIVE_DIR"
SLUG_TITLE=$(echo "<title>" | tr ' ' '-' | tr -cd 'a-zA-Z[object Object]-9-' | tr A-Z a-z | cut -c[object Object]-6[object Object])
...
**E2 TOCTOU re-check (F[object Object]):** After the user answers, IMMEDIATELY re-run
...
**D[object Object]6 mandatory final-confirm gate:** AskUserQuestion: "Spawn agent now? Last
...
```bash
git worktree add "$SPAWN_PATH" -b "$SPAWN_BRANCH" "$PIN_SHA" 2>&[object Object]
```
...
```bash
cat "$ARCHIVE_PATH" | (cd "$SPAWN_PATH" && claude -p 2>&[object Object]) &
SPAWN_PID=$!
...
#### TTHW telemetry (DX[object Object][object Object]/F7)

Capture timestamps at three checkpoints, write to telemetry envelope at /spec
exit:

- `T_PHASE[object Object]_START` — Phase [object Object] first AskUserQuestion or first text emit
...
end-of-skill telemetry write emits, as `ttfc_ms` (Phase [object Object] → first citation) and
`tthw_ms` (Phase [object Object] → file/spawn) JSON fields. Surfacing the aggregates in
...
### [object Object]. Stakeholder Context ("Why This Matters")
...
before/after. "Several files" → "47 files across [object Object]2 directories." "Improves
performance" → "reduces query from ~5[object Object][object Object]ms to ~5[object Object]ms ([object Object][object Object]x)." If you lack numbers,
...
```
#[object Object] Foundation ─┬─> #2 Core Feature A
...
### [object Object][object Object]. Testable Acceptance Criteria

Numbered. Pass/fail. No subjective language.

- ✅ "Orders older than 3[object Object] days return HTTP 4[object Object][object Object] for all 4 user roles"
- ✅ "Query time for [object Object][object Object]K-row table under [object Object][object Object][object Object]ms (EXPLAIN ANALYZE)"
- ❌ "The feature works correctly"
- ❌ "Edge cases are handled"

### [object Object][object Object]. Testing Pyramid
...
| Integration | Create order → expire → verify 4[object Object][object Object] | +2    |
| E2E         | Login → view orders → see expired  | +[object Object]    |
```

### [object Object]2. Root Cause Analysis (bugs and quality issues)
...
### [object Object]3. Effort Breakdown

Per-component, not just a total. "~[object Object]2h" → "2h schema + 3h service + 4h tests +
3h frontend." Enables planning and task splitting.

### [object Object]4. Rollback Strategy
...
[object Object]. [Specific, pass/fail, no subjective language]
2. [...]
...
[object Object]. [Numbered, specific, measurable verification checkpoints]
```
...
[object Object]. **NEVER produce an issue after the first message.** Always start with Phase [object Object].
...
5. **Flag when something should be multiple issues.** Propose epic + children if scope
   has natural seams. Individual issues should be completable in [object Object]-3 days.
...
9. **Explain sequencing.** Don't just list priorities — explain what makes Critical
   vs Medium, and why Phase [object Object] precedes Phase 2.
...
- 2[object Object]+ items in one issue without severity tiers and execution plan
