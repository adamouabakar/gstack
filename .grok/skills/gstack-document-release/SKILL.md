...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
## Step [object Object]: Pre-flight & Diff Analysis

[object Object]. Check the current branch. If on the base branch, **abort**: "You're on the base branch. Run from a feature branch."
...
## Step [object Object].5: Coverage Map (Blast-Radius Analysis)
...
[object Object]. **Extract public surface changes from the diff.** Scan `git diff <base>...HEAD` for:
...
- **Ask user** — Narrative changes, section removal, security model changes, large rewrites
  (more than ~[object Object][object Object] lines in one section), ambiguous relevance, adding entirely new sections.
...
from 9 to [object Object][object Object]."
...
**Rules:**
[object Object]. Read the entire CHANGELOG.md first. Understand what is already there.
...
- **Sell test (Diataxis rubric):** Score each CHANGELOG entry [object Object]-3:
  - **[object Object] point** — answers "What changed?" (reference: names the feature/fix)
  - **[object Object] point** — answers "Why should I care?" (explanation: user impact, pain removed)
  - **[object Object] point** — answers "How do I use it?" (how-to: command, flag, or link to docs)
...
[object Object]. Does the README's feature/capability list match what CLAUDE.md (or project instructions) describes?
...
[object Object]. **Completed items not yet marked:** Cross-reference the diff against open TODO items. If a
...
[object Object]. **If VERSION does not exist:** Skip silently.
...
3. **If VERSION was NOT bumped:** Use AskUserQuestion:
   ...
   - A) Bump PATCH (X.Y.Z+[object Object]) — if doc changes ship alongside code changes
   - B) Bump MINOR (X.Y+[object Object].[object Object]) — if this is a significant standalone release
...
4. **If VERSION was already bumped:** Do NOT skip silently. Instead, check whether the bump
   ...
   d. **If there are significant uncovered changes:** Use AskUserQuestion explaining what the
      ...
      - A) Bump to next patch (X.Y.Z+[object Object]) — give the new changes their own version
...
[object Object]. Stage modified documentation files by name (never `git add -A` or `git add .`).
...
[object Object]. Read the existing PR/MR body into a PID-unique tempfile (use the platform detected in Step [object Object]):
...
3. The Documentation section should include:
   ...
   a. **Doc diff preview** — for each file modified, describe what specifically changed (e.g.,
      "README.md: added /document-release to skills table, updated skill count from 9 to [object Object][object Object]").

   b. **Documentation debt** — if the coverage map from Step [object Object].5 found gaps, append a
...
~/.grok/skills/gstack/bin/gstack-redact --from-file /tmp/gstack-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
...
[object Object]. Read the current VERSION:
...
```bash
NEW_TITLE=$(~/.grok/skills/gstack/bin/gstack-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
...
If the coverage map from Step [object Object].5 identified any gaps, append:
...
> doc review only reports; nothing is edited without your say-so. Completeness: A=9/[object Object][object Object], B=4/[object Object][object Object], C=8/[object Object][object Object].
...
**Persist the result:**
```bash
~/.grok/skills/gstack/bin/gstack-review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
