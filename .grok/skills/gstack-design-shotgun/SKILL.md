...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-shotgun","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
## Step [object Object]: Session Detection
...
If B: proceed to Step [object Object].
...
## Step [object Object]: Context Gathering
...
[object Object]. **Who** — who is the design for? (persona, audience, expertise level)
...
```bash
cat DESIGN.md 2>/dev/null | head -8[object Object] || echo "NO_DESIGN_MD"
```
...
```bash
curl -s -o /dev/null -w "%{http_code}" http://localhost:3[object Object][object Object][object Object] 2>/dev/null || echo "NO_LOCAL_SITE"
```
...
**Persistent taste profile (v[object Object] schema at `~/.gstack/projects/$SLUG/taste-profile.json`):**
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find ~/.gstack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -[object Object][object Object])
```
...
Limit to last [object Object][object Object] sessions. Try/catch JSON parse on each (skip corrupted files).
...
> "These are the {N} directions I'll generate. Each takes ~6[object Object]s, but I'll run them all
> in parallel so total time is ~6[object Object] seconds regardless of count."
...
`DESIGN_READY: /path/to/design` output in Step [object Object]) into each agent prompt.
...
Steps:
[object Object]. Run: {$D path} generate --brief "{brief}" --output /tmp/variant-{letter}.png
...
For the evolve path, replace step [object Object] with:
```
...
[object Object]. Read each generated PNG inline (Read tool) so the user sees all variants at once.
...
[object Object]. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
