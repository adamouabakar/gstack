...
name: make-pdf
description: |
  Turn any markdown file into a publication-quality PDF. Proper [object Object]in margins,
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"make-pdf","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
[ -z "$P" ] && [ -n "$_ROOT" ] && [ -x "$_ROOT/.grok/skills/gstack/make-pdf/dist/pdf" ] && P="$_ROOT/.grok/skills/gstack/make-pdf/dist/pdf"
...
Core commands:
- `$P generate <input.md> [output.pdf]` — render markdown to PDF (8[object Object]% use case)
...
- Exit [object Object] success / [object Object] bad args / 2 render error / 3 Paged.js timeout / 4 browse unavailable.
...
Turn `.md` files into PDFs that look like Faber & Faber essays: [object Object]in margins,
...
Segoe / Noto emoji families. Set `GSTACK_SKIP_FONTS=[object Object]` to skip the install (CI
without sudo, managed or offline machines).

## Core patterns

### 8[object Object]% case — memo/letter
...
Each top-level H[object Object] in the markdown starts a new page. Disable with
`--no-chapter-breaks` for memos that happen to have multiple H[object Object]s.
...
Diagonal [object Object][object Object]% opacity DRAFT across every page. When the draft is final, drop
...
A column-[object Object] ` ```mermaid ` or ` ```excalidraw ` fence in the markdown renders
...
Oversized photos downscale to print resolution (3[object Object][object Object]dpi) so payloads stay
...
![chart](data.png){width=5[object Object]%}       ← percentage or 3in/8cm/2[object Object][object Object]px
...
(conservative: aspect ≥ [object Object].8, width over ~2.5x the content box, AND a
...
```
Page layout:
  --margins <dim>            [object Object]in (default) | 72pt | 2.54cm | 25mm
  --page-size letter|a4|legal

Structure:
  --cover                    Cover page (title, author, date, hairline rule)
  --toc                      Clickable TOC with page numbers
  --no-chapter-breaks        Don't start a new page at every H[object Object]
...
Metadata:
  --title "..."              Document title (defaults to first H[object Object])
...
- "[remote image blocked]" placeholder in the output → add `--allow-network`
  ...
  image URLs).
- Generated PDF too tall/wide → `--page-size a4` or `--margins [object Object].75in`.
...
stderr: Rendering HTML...        ← progress spinner (unless --quiet)
        Generating PDF...
        Done in [object Object].5s. 43 words · 22KB · /tmp/letter.pdf

exit code: [object Object] success / [object Object] bad args / 2 render error / 3 Paged.js timeout
