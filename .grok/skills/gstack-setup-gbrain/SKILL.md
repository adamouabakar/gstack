...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"setup-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z[object Object]-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
## Step [object Object]: Detect current state

```bash
~/.grok/skills/gstack/bin/gstack-gbrain-detect
```
...
the v[object Object].34.[object Object].[object Object]+ `gbrain_local_status` field (one of: `ok`, `no-cli`,
...
> "Detected: gbrain v[object Object].[object Object]8.2 on PATH, engine=postgres, doctor=ok,
...
## Step [object Object].5: Broken-local-engine remediation (plan D4)

Read `gbrain_local_status` from the Step [object Object] detect output. **If it's `broken-db`
...
> ELI[object Object][object Object]: gbrain has a config at `~/.gbrain/config.json` but the engine it points
...
> A) Retry — re-probe the engine (recommended; ~8[object Object]ms)
...
**If A (Retry)**: re-run `~/.grok/skills/gstack/bin/gstack-gbrain-detect`
with `GSTACK_DETECT_NO_CACHE=[object Object]` (busts the 6[object Object]s cache). If the new
...
# gstack default: voyage-code-3 ([object Object][object Object]24d) when VOYAGE_API_KEY is set — best for
# code retrieval. Without the key, fall back to gbrain's own auto-selected
# embedding provider chain (OpenAI [object Object]536d when OPENAI_API_KEY is present, etc.).
GBRAIN_EMBED_FLAGS=""
if [ -n "${VOYAGE_API_KEY:-}" ]; then
  GBRAIN_EMBED_FLAGS="--embedding-model voyage:voyage-code-3 --embedding-dimensions [object Object][object Object]24"
fi
...
Step [object Object].5 — fall through to Step 2 (where `no-cli` triggers Step 3 install and
...
Only fire this if Step [object Object] shows no existing working config AND no shortcut
...
- **[object Object] — Supabase, I already have a connection string.** Cloud-agent users
...
- **2a — Supabase, auto-provision a new project.** You'll need a Supabase
  Personal Access Token (~9[object Object] seconds). Best choice for a shared team brain.
- **2b — Supabase, create manually.** Walk through supabase.com signup
  yourself; paste the URL back when ready.
- **3 — PGLite local.** Zero accounts, ~3[object Object] seconds. Isolated brain on this
...
- **Switch** (only if Step [object Object] detected an existing engine): "You already have
  a `<engine>` brain. Migrate it to the other engine?" → runs
  `gbrain migrate --to <other>` wrapped in `timeout [object Object]8[object Object]s` (D9).
...
For Paths [object Object], 2a, 2b, 3, switch — only if `gbrain_on_path=false`:

```bash
~/.grok/skills/gstack/bin/gstack-gbrain-install
```

The installer runs D5 detect-first (probes `~/git/gbrain`, `~/gbrain` first),
then D[object Object]9 PATH-shadow validation (post-link `gbrain --version` must match
install-dir `package.json`). On D[object Object]9 failure the installer exits 3 with a
...
### Path [object Object] (Supabase, existing URL)

Source the secret-read helper, collect URL with `read -s` + redacted preview:

```bash
. ~/.grok/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.grok/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
On success, hand off to gbrain via env var (D[object Object][object Object], never argv):
...
now persisted in `~/.gbrain/config.json` at mode [object Object]6[object Object][object Object] by gbrain itself.

### Path 2a (Supabase, auto-provision — D7)

Show the D[object Object][object Object] PAT scope disclosure verbatim BEFORE collecting the token:
...
Ask the D[object Object]7 tier prompt via AskUserQuestion: "Which Supabase tier?" Present
...
```bash
orgs=$(~/.grok/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
Ask the user for a region (default `us-east-[object Object]`; valid values are the [object Object]8
...
Set up a SIGINT trap (D[object Object]2 basic recovery):
...
trap 'echo ""; echo "gstack-gbrain: interrupted. In-flight ref: $INFLIGHT_REF"; \
      ...
      unset SUPABASE_ACCESS_TOKEN DB_PASS; exit [object Object]3[object Object]' INT TERM
```

Create + wait + fetch:

```bash
result=$(~/.grok/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.grok/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.grok/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
[object Object]. Login at https://supabase.com/dashboard
...
Then follow the same secret-read + verify + init flow as Path [object Object].

### Path 3 (PGLite local)

```bash
# gstack default: voyage-code-3 ([object Object][object Object]24d) when VOYAGE_API_KEY is set — code
# retrieval beats general-purpose embeddings on real code queries (validated
# A/B). Without the key, gbrain auto-selects (OpenAI [object Object]536d when available).
GBRAIN_EMBED_FLAGS=""
if [ -n "${VOYAGE_API_KEY:-}" ]; then
  GBRAIN_EMBED_FLAGS="--embedding-model voyage:voyage-code-3 --embedding-dimensions [object Object][object Object]24"
fi
...
`VOYAGE_API_KEY` is set — ~$[object Object].[object Object]8 per [object Object]M tokens, pennies per repo).
...
```
Paste your gbrain MCP URL (e.g. https://wintermute.tail554574.ts.net:3[object Object]3[object Object]/mcp):
```
...
**4b. Collect bearer token via the secret-read helper (D[object Object][object Object], never argv).**
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.grok/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
- `SERVER_VERSION` (e.g., `[object Object].27.[object Object]`) — written to the CLAUDE.md block in Step 8.
...
**4d. (Path 4) Offer local PGLite for code search.** Per plan D[object Object][object Object]/D[object Object][object Object], ask:

> D# — Want symbol-aware code search on this machine?
> Project/branch/task: <one-sentence grounding using detected slug + branch>
> ELI[object Object][object Object]: The remote brain at `<MCP_URL>` is great for cross-machine knowledge,
...
> database (~3[object Object] seconds, no accounts, ~[object Object]2[object Object] MB disk) just for code, separate
...
> back to Grep.
> Recommendation: A — 3[object Object] seconds, no ongoing cost, unlocks the symbol tools.
> Completeness: A=[object Object][object Object]/[object Object][object Object] (full split-engine), B=7/[object Object][object Object] (remote-only).
...
```bash
~/.grok/skills/gstack/bin/gstack-gbrain-install || exit $?
...
# gstack default for local code-search PGLite: voyage-code-3 ([object Object][object Object]24d) when
...
installed). `/sync-gbrain` will SKIP the code stage cleanly per plan D[object Object]2.
...
per plan D[object Object][object Object].
...
resting state in `~/.claude.json` mode [object Object]6[object Object][object Object].

### Switch (from detect's existing-engine state)

```bash
# Going PGLite → Supabase, collect URL first (Path [object Object] flow), then:
timeout [object Object]8[object Object]s gbrain migrate --to supabase --url "$URL" --json
# Going Supabase → PGLite:
timeout [object Object]8[object Object]s gbrain migrate --to pglite --json
```

If `timeout` returns [object Object]24 (exit code for timeout): surface D9 message
...
For Paths [object Object], 2a, 2b, 3, switch:
...
## Step 5a: Register gbrain as Claude Code MCP (D[object Object]8)
...
~[object Object][object Object]ms. The token's resting state is `~/.claude.json` (mode [object Object]6[object Object][object Object] — Claude
...
### Paths [object Object], 2a, 2b, 3 (Local stdio)
...
```bash
current_tier=$(~/.grok/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.grok/skills/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
Renamed from "session memory sync" in v[object Object].27.[object Object].[object Object] — the on-disk concept is
...
verify output (Path 4) or `false` (Paths [object Object]/2/3 — local mode doesn't probe):

```bash
URL_FORM=${URL_FORM_SUPPORTED:-false}
~/.grok/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.grok/skills/gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
### Paths [object Object], 2a, 2b, 3 (Local stdio) — wire up the federated source
...
try:
    ...
    pass
")
~/.grok/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
  ${GBRAIN_URL:+--database-url "$GBRAIN_URL"}
```

`--strict` exits non-zero on missing prereqs (gbrain not installed, < [object Object].[object Object]8.[object Object],
...
For Paths [object Object], 2a, 2b, 3:
...
```bash
~/.grok/skills/gstack/bin/gstack-memory-ingest --probe
```

Read the output. If `Total files in window: [object Object]`, skip — there's nothing
to ingest. Set `gstack-config set transcript_ingest_mode incremental`
silently and continue to Step 8.

If `New (never ingested)` is < 2[object Object][object Object] AND total bytes are < [object Object][object Object][object Object]MB: silent
...
only, last 9[object Object] days**:

> "Found <N_repo> transcripts in THIS repo (<repo-slug>) over the last
> 9[object Object] days, plus <N_other> across other repos on this machine (<bytes>
...
Options:
- A) Yes — this repo, last 9[object Object] days (recommended; ~est min)
...
After answer:
```bash
~/.grok/skills/gstack/bin/gstack-config set transcript_ingest_mode <choice>
~/.grok/skills/gstack/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
- Config file: ~/.gbrain/config.json (mode [object Object]6[object Object][object Object])
- Setup date: {today}
...
To verify from the shell right now (without waiting for restart):
  curl -s -X POST -H 'Content-Type: application/json' \
       ...
       -d '{"jsonrpc":"2.[object Object]","id":[object Object],"method":"tools/list","params":{}}' \
       <YOUR_MCP_URL>
```
...
## Step 9.5: Brain trust policy (v[object Object].48 brain-aware planning, D4 / Phase [object Object].5)
...
```bash
_HASH=$(~/.grok/skills/gstack/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.grok/skills/gstack/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
"Trust policy for this endpoint: $_POLICY" and skip to Step [object Object][object Object].

**If `_POLICY` is `unset` AND `_HASH == "local"`:** auto-set personal
(local engines are inherently single-tenant). No AskUserQuestion.

```bash
~/.grok/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
~/.grok/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.grok/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.grok/skills/gstack/bin/gstack-config set artifacts_sync_mode full
...
## Step [object Object][object Object]: GREEN/YELLOW/RED verdict block (idempotent doctor output)

After Steps [object Object]-9 complete, summarize. Re-running `/setup-gbrain` on a
...
```bash
~/.grok/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.grok/skills/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.grok/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
...
gbrain status: GREEN  (mode: remote-http)
  ...
  Transcripts ..... OK   route to artifacts repo → remote brain (plan D[object Object][object Object])
...
The **Transcripts** row changed in v[object Object].34.[object Object].[object Object]: in remote-http mode,
...
`Engine .......... ERR  PGLite corrupt — run \`gbrain restore-from-sync\` (V[object Object].5)`).
For V[object Object], restore-from-sync is a V[object Object].5 P[object Object] cross-repo TODO; until it ships,
...
## `/setup-gbrain --cleanup-orphans` (D2[object Object])
...
projects=$(curl -s -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" \
  https://api.supabase.com/v[object Object]/projects)
```
...
curl -s -X DELETE -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" \
  https://api.supabase.com/v[object Object]/projects/$REF
```
...
- **One rule for every secret.** PAT, DB_PASS, pooler URL: env-var only,
  ...
  by gbrain's own `init` at mode [object Object]6[object Object][object Object] — that's gbrain's discipline, not
  ours.
- **STOP points are hard.** Gbrain doctor not healthy, D[object Object]9 PATH shadow, D9
